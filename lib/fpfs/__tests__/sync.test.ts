...
describe("syncCampeonato", () => {
  ...
  it("recria a classificacao e marca ehNosso pelo fpfsTimeNome", async () => {
    ...
    expect(rows.find((r: { timeNome: string; ehNosso: boolean }) => r.timeNome === "E.C. Itaquerense").ehNosso).toBe(true)
    expect(rows.find((r: { timeNome: string; ehNosso: boolean }) => r.timeNome === "Vila Real").ehNosso).toBe(false)
  })
